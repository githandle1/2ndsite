...
function rasterContain(img, cells) {
  const canvas = document.createElement("canvas");
  canvas.width = cells;
  canvas.height = cells;
  const ctx = canvas.getContext("2d", { willReadFrequently: true });
  ctx.clearRect(0, 0, cells, cells);
  const scale = Math.min(cells / img.width, cells / img.height);
  const w = img.width * scale;
  const h = img.height * scale;
  ctx.drawImage(img, (cells - w) / 2, (cells - h) / 2, w, h);
...
export async function buildPhotoPaint({ photo, seed = 1, size = 720, effects = null }) {
  ...
  const marks = planMarks(data, CELLS, size, rand, wet, e.brushType || "HB");
  const brushName = e.brushType || "HB";
