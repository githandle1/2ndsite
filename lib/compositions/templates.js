...
export const templates = {
  ...
  brush.noStroke();
  brush.fill("#e4d2be", 72);
  brush.fillBleed(0.55);
  brush.fillTexture(0.22, 0.12);
  brush.circle(400, 400, 420, 0.78);

  const prints = [
    { x: 210, y: 250, a: -18, s: 1.55 },
    { x: 590, y: 280, a: 22, s: 1.42 },
    { x: 250, y: 560, a: 10, s: 1.38 },
    { x: 560, y: 580, a: -12, s: 1.34 }
  ];
  const inks = ["#6b3a32", "#8a4a3c", "#5c322c", "#7a4034"];

  for (let p = 0; p < prints.length; p++) {
    const print = prints[p];
    const rad = radians(print.a);
    for (let r = 6; r >= 1; r--) {
      brush.fill(inks[p % inks.length], 88 + r * 14);
      brush.fillBleed(0.2);
      brush.fillTexture(0.38, 0.3);
      const pts = [];
      const rx = (44 + r * 28) * print.s;
      const ry = (58 + r * 36) * print.s;
      for (let i = 0; i < 16; i++) {
    ...
    brush.fill("#4a241f", 96);
    brush.fillBleed(0.12);
    brush.circle(print.x, print.y + 14 * print.s, 28 * print.s, 0.58);
  }

  brush.set("HB", "#5a332c", 0.72);
  for (let i = 0; i < 11; i++) {
    const a = radians(-74 + i * 15);
    brush.line(175, 175, 175 + Math.cos(a) * (88 + random(18)), 175 + Math.sin(a) * (40 + random(14)));
  }
}`,
