...
export const templates = {
  ...
  brush.noStroke();
  brush.fill("#e8d8c6", 40);
  brush.fillBleed(0.5);
  brush.fillTexture(0.18, 0.1);
  brush.circle(400, 420, 340, 0.85);

  const prints = [
    { x: 270, y: 330, a: -18, s: 1.15 },
    { x: 530, y: 370, a: 22, s: 1.06 },
    { x: 400,  y: 540, a: 8, s: 0.98 }
  ];
  ...
  for (let p = 0; p < prints.length; p++) {
    ...
    for (let r = 6; r >= 1; r--) {
      brush.fill(inks[p % inks.length], 28 + r * 8);
      brush.fillBleed(0.22);
      brush.fillTexture(0.35, 0.28);
      const pts = [];
      const rx = (30 + r * 22) * print.s;
      const ry = (40 + r * 28) * print.s;
      for (let i = 0; i < 16; i++) {
    ...
    brush.circle(print.x, print.y + 10 * print.s, 18 * print.s, 0.6);
  }

  brush.set("HB", "#5a332c", 0.55);
  for (let i = 0; i < 9; i++) {
    const a = radians(-70 + i * 16);
    brush.line(255, 270, 255 + Math.cos(a) * (58 + random(14)), 270 + Math.sin(a) * (28 + random(10)));
  }
}`,
