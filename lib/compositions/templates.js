export const samples = [
  ...
  },
  {
    id: "littleprince",
    title: "little prince",
    prompt: "the little prince on his small planet",
    color: "#c49646",
    brushType: "HB",
  },
];
...
export const templates = {
  ...
  littleprince: `function paint() {
  brush.noStroke();
  brush.fill("#d8dce8", 36);
  brush.fillBleed(0.52);
  brush.fillTexture(0.34, 0.22);
  brush.circle(400, 220, 260, 0.86);

  brush.fill("#c5c0d4", 28);
  brush.fillBleed(0.4);
  brush.circle(250, 180, 140, 0.8);
  ...
  for (let i = 0; i < stars.length; i++) {
    brush.fill("#f4ead4", 64 + random(-8, 14));
    brush.fillBleed(0.1);
    brush.circle(stars[i][0] + random(-5, 5), stars[i][1] + random(-5, 5), 3 + random(4), 0.55);
  }

  brush.fill("#d2b07a", 58);
  brush.fillBleed(0.42);
  brush.fillTexture(0.4, 0.26);
  brush.circle(400, 520, 210, 0.72);

  brush.fill("#c49646", 88);
  brush.fillBleed(0.28);
  brush.fillTexture(0.42, 0.3);
  const planet = [];
  for (let i = 0; i < 20; i++) {
    ...
    planet.push([
      400 + Math.cos(t) * (188 + random(-8, 10)),
      528 + Math.sin(t) * (168 + random(-7, 8))
    ]);
  }
  brush.polygon(planet);

  brush.fill("#a87a38", 70);
  brush.fillBleed(0.2);
  brush.circle(330, 560, 46, 0.62);
  brush.circle(470, 500, 34, 0.58);
  brush.fill("#8a6230", 55);
  brush.circle(430, 600, 28, 0.5);

  brush.fill("#6b8a4a", 72);
  brush.fillBleed(0.22);
  brush.polygon([
    [348, 430],
    [368, 390],
  ...
  brush.fill("#c45b5f", 80);
  brush.fillBleed(0.26);
  brush.circle(378, 378, 11, 0.55);

  brush.fill("#3f6a58", 110);
  ...
  brush.fill("#2f5346", 70);
  brush.circle(408, 428, 16, 0.4);

  brush.fill("#f0d7a8", 120);
  brush.fillBleed(0.12);
  brush.circle(408, 348, 18, 0.5);
  brush.fill("#e4c46a", 90);
  brush.circle(404, 340, 16, 0.48);

  brush.fill("#6e8eba", 78);
  brush.fillBleed(0.2);
  const scarf = [];
  for (let i = 0; i <= 8; i++) {
    const t = i / 8;
    scarf.push([424 + t * 54 + random(-2, 2), 366 + t * 8 + Math.sin(t * 6) * 6]);
  }
  for (let i = 8; i >= 0; i--) {
    const t = i / 8;
    scarf.push([424 + t * 54 + random(-2, 2), 378 + t * 10 + Math.sin(t * 6) * 6]);
  }
  brush.polygon(scarf);

  brush.set("HB", "#5a4a32", 0.42);
  brush.line(400, 430, 392, 468);
  brush.line(416, 430, 424, 468);
  brush.line(424, 372, 478, 380);
}`,
};
